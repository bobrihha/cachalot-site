...
const ChatWidget = () => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { text: "Здравствуйте! Я цифровой помощник. Задайте вопрос, и я найду ответ в нашей базе знаний.", isUser: false }
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { text: "Ошибка соединения с сервером (возможно блокировка HTTP).", isUser: false }]);
        } finally {
            setIsLoading(false);
        }
    };
                                ...
                                <div key={idx} className={`flex ${msg.isUser ? 'justify-end' : 'justify-start'}`}>
                                    ...
                                    >
                                        <ReactMarkdown
                                            ...
                                            components={{
                                                a: ({ node, ...props }) => <a {...props} target="_blank" rel="noopener noreferrer" className="text-neon-cyan underline break-all" />
                                            }}
                                        >
                                            {msg.text}
                                        </ReactMarkdown>
